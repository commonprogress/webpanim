...
public class FrameSequenceDrawable extends Drawable implements Animatable, Runnable {
    ...
    public FrameSequenceDrawable(InputStream inputStream, boolean isAutoPlay) {
    ...
    public void draw(Canvas canvas) {
        synchronized (mLock) {
            ...
            if (isRunning() && mState == STATE_READY_TO_SWAP) {
                ...
                if (mNextFrameToDecode == getFrameSequenceCount() - 1) {
                    mCurrentLoop++;
    ...
    private void scheduleDecodeLocked() {
        ...
        mNextFrameToDecode = (mNextFrameToDecode + 1) % getFrameSequenceCount();
        sDecodingThreadHandler.post(mDecodeRunnable);
    }

    public int getFrameSequenceCount() {
        return null == mFrameSequence ? 0 : mFrameSequence.getFrameCount();
    }
